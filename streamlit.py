...
# Cấu hình trang
st.set_page_config(page_title="📊 Dashboard Doanh Thu", page_icon="💼", layout="wide")
...
with tab2:
    if show_chart:
        ...
        fig = px.line(df, x="Tháng", y="Doanh thu", markers=True, title="Doanh thu theo tháng")
...
with tab3:
    ...
    st.metric(label="📌 Tổng doanh thu", value=f"${df['Doanh thu'].sum():,.0f}")
    st.metric(label="📈 Doanh thu cao nhất", value=f"${df['Doanh thu'].max():,.0f}")
    st.metric(label="📉 Doanh thu thấp nhất", value=f"${df['Doanh thu'].min():,.0f}")

# Cấu hình giao diện
st.set_page_config(page_title="Dự báo nhu cầu", layout="wide")
...
with st.spinner("🔄 Đang tải dữ liệu từ GitHub..."):
    try:
        ...
        if response.status_code == 200:
            ...
            st.error("❌ Không thể tải dữ liệu. Kiểm tra lại link.")
    ...
    category_sales.plot(kind='pie', autopct='%1.1f%%', startangle=90, ax=ax2)
    ax2.set_ylabel('')
    st.pyplot(fig2)
